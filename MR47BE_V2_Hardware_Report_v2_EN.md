...
**test4 target:** mainline has no *DSA-style* switch driver for QCA8084; the initial goal is UART + Wi-Fi (ath12k) +
4× PHY link-up. LAN switch/VLAN functionality is separate DSA-driver work.
*(Note: the Ethernet datapath itself is driven by mainline `kmod-qcom-ppe`; "no DSA driver" refers to a
DSA switch node, not to the Ethernet datapath.)*
