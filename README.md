...
# MR47BE V2 — HARDWARE STATUS (FINAL, evidence-based)

**Date:** 2026-08-28 · **Unit:** MR47BE(EU) Ver:2.0 · Serial **2CF60F2000345**
**Method:** Direct chip-top photo reading + real UART PBL/SBL/U-Boot boot log + vendor GPL
(`kernel-ipq5332-mi01.6.dts`, `nand-partition.xml`, `basic.config`).
This supersedes any earlier assumption (incl. the IPQ5332 / QCA8386 guesses).

> ✅ = confirmed by photo **and/or** boot log. 🟡 = open. ❌ = disproven.

---

## 1. Bill of Materials — CONFIRMED

| Component | Photo evidence (resimler/) | Boot-log evidence | Status |
|---|---|---|---|
| **Main SoC** | `BOTTOM_Qalcomm_IPQ5322_003_FK5134HJ*.jpg` (5 img — `IPQ5322`) | `JTAG ID=0x1023d0e1` (QCOM vendor 0x70) | ✅ **IPQ5322** |
| **6 GHz radio** | `BOTTOM_Qalcomm_QCN6274_001_JE510CV2*.jpg` | PCIe link up (`QCN9224/QCN6274` path) | ✅ QCN6274 |
| **NAND flash** | `TOP_winbond_25N01GWZE1G*.jpg` | `W25N01GWZEIG ... 128 MiB` (ID 0xef 0x21ba) | ✅ exact |
| **DRAM** | `BOTTOM_SDRAM_SEC_343_K4A8G165WC_BCTD*.jpg` | `DRAM: ... 1 GiB` | ✅ exact |
| **LAN magnetics (4×2.5G)** | `BOTTOM_EEC_DQ48201N0-S_2520A_LAN Transformer*.jpg` | — | ✅ |
| **UART header** | `TOP_UART_CONNECT*.jpg` (4-pin J1) | working capture | ✅ |
| Antenna feeds (5× U.FL) | `TOP __.jpg` etc. | — | ✅ |

**SoC clarification:** `IMAGE_VARIANT_STRING=IPQ5332LA` in the boot log is a *BSP compiler
string*, NOT a hardware read. The physical die reads **IPQ5322** (Immersive Home 326,
6-stream) — a real, distinct SKU in the same Miami family as IPQ5332 (10-stream).

---

## 2. Ethernet — CONFIRMED: **QCA8084** (QCA8386 ❌ DISPROVEN)

Boot log states directly:
```
eth0 PHY0 QCA8084-switch status:
PORT1 Up Speed :100 Full duplex     <- port with active traffic
PORT2 Down ...
PORT3 Down ...
eth0 PHY1 Down <>
```
**About those speed values:** `:100` / `:10` are the **negotiated (auto-neg) link
speeds at capture time** — the unit was connected to the internet through a
**TP-Link 740N (a 100 Mbps device)**, NOT the MR47's native port speed. All 4
Ethernet ports are **2.5G-capable** (LAN magnetics `EEC DQ48201N0-S` ×2 = 4×2.5GbE);
to see 2.5G you must connect a 2.5G/5G-capable NIC and read `Link: 2500 Mbps`.

**The only hard hardware fact from this log is the string `QCA8084-switch`** — the
Ethernet solution is a **QCA8084** switch (this is what UART shows in place of the
missing QCA8386): **LAN PORT1..3 + WAN PHY1**.
Vendor DTS confirms the wiring: `dp1` (`qcom,id=2`, `is_switch_connected`) + `dp2`
(`qcom,id=1`), mdio `phy0..3`.

> OpenWrt/mainline has **no DSA driver** for QCA8084 as a *switch* (same as QCA8386).
> Realistic first goal: 4× PHY link up + UART + Wi-Fi; switch/VLAN is a driver task.

---

## 3. Flash / Partitioning — CONFIRMED (nand-partition.xml)

Total 128 MiB A/B layout: sbl1 768K · mibib 512K · bootconfig(+1) 256K×2 · qsee 1.75M ·
...
ART holds Wi-Fi calibration + MAC → must stay read-only.

---

## 4. Security / Boot — CONFIRMED

- `Secure Boot: Off` → **unsigned OpenWrt RAM-boot is possible**.
- U-Boot `IPQ5332#`, 115200, FIT signature/SHA/RSA disabled.
...
| # | Item | Status |
|---|---|---|
| 1 | SoC **IPQ5322** (photo-proof, this unit) | ✅ |
| 2 | Ethernet **QCA8084** switch-PHY (boot log) | ✅ |
| 3 | QCA8386 present? | ❌ NO |
| 4 | NAND W25N01GWZEIG 128M / 1 GiB DDR4 | ✅ |
| 5 | on-SoC 2.4/5 GHz + QCN6274 6 GHz (ath12k) | ✅ |
| 6 | UART J1 working, Secure Boot Off | ✅ |

**Verdict:** Hardware is fully identified. test4 image must be built against **IPQ5322 +
QCA8084**, not the old QCA8386/IPQ5332 assumptions. Current test1/2/3 images do NOT match.
