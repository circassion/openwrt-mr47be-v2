...
|---|---|---|
| USB kaldırıldı | ✅ | Fiziksel USB girişi yok → `&usb`/`&usbphy0` DTS'ten silindi, USB paketleri `ipq53xx.mk`'den çıkarıldı |
| LED | ✅ | gpio-leds (wan/lan1/lan2/lan3/sys) — test4 DTS'te ZATEN mevcuttu (doğrulandı) |
| Button | ✅ | gpio-keys (reset @ tlmm30) — test4 DTS'te ZATEN mevcuttu (doğrulandı) |
| Kernel | ✅ | 6.18.39 (kalıcı; OpenWrt qualcommbe LTS) |
...
| `imajlar/test5/` | ⏳ test4 kopyası ESKİ/dönemsel — gerçek build çıktısı ile DEĞİŞTİRİLECEK |

### İmajlar

> ✅ Gerçek test5 build'i (21:12) `bin/targets/qualcommbe/ipq53xx/`'ten buraya kopyalandı.
> SHA256'lar test4'ten FARKLI — kopya değil, gerçek yeniden derleme.
...
### Build Durumu — ✅ TAMAMLANDI (2026-08-29, 21:12)

- **Build:** `cd /root/mr47be-clean && setsid nohup make -j4 V=s > /root/mr47be-clean-build.log 2>&1 < /dev/null &`
- **Kernel:** **6.18.39~bc29be76** (test4'ün `6.18.39~1211adca` versiyonundan FARKLI — gerçek yeniden derleme)
- **USB:** ✅ **kernel USB desteği tamamen kapatıldı** (`CONFIG_USB_SUPPORT=n` → `drivers/usb/*.ko` = 0),
  DTS'te `&usb`/`&usbphy0` silindi, USB paketleri `ipq53xx.mk`'den çıkarıldı. Fiziksel port yok.
  (Not: OpenWrt paket metadata'sı boş `kmod-usb-*` etiketleri + `usbutils` CLI aracı bırakır —
  bunlar modül İÇERMEZ, cihazda USB işlevi YOKTUR.)
- **LED/Button:** test4'te de vardı; test5'te korundu (gpio-leds 6× + gpio-keys reset).
- **İmajlar:** `bin/targets/.../ipq53xx/` → **`imajlar/test5/`** kopyalandı.
  SHA256'lar test4'ten **FARKLI** (kanıt: initramfs `944038f9→9e8af2d8`, sysupgrade `0bd575ff→44f79ae7`).
- **Doğrulama:** fotoğraf + `V2_HARDWARE_STATUS*` + `MR47BE_V2_Hardware_Report_v2` ile
  SoC=**IPQ5322**, WiFi=**QCN6274**, Ethernet=**QCA8084** çapraz-doğrulandı.
- **Not:** `.44/.48`'e çıkma girişimi patch uyuşmazlığı ile BAŞARISIZ → 6.18.39 kaldı (bkz. Ders 5).
...
### Ders 5: Kernel Sürüm Çıkarımı — Patch Tuzağı (2026-08-29)

**Sorun:** Dokümanlar "kernel 6.18.48" diyordu; gerçek build 6.18.39 üretiyordu.
Ağaç `target/linux/generic/kernel-6.18` → `LINUX_VERSION-6.18 = .39` idi. `.48`e
çıkınca (hatta OpenWrt üst ana dalının resmi sürümü `.44`e) `backport-6.18/*.patch`
uygulanmayor → build öldü. Kanıt: `300-v7.1-MIPS-mm-fix-highmem-init.patch` Hunk #1
FAILED.

**Kural:** .39→.44/.48'e sadece sürüm satırını değiştirerek çıkılamaz; OpenWrt'in
`backport-6.18/` + `patches-6.18/` seti belirli nokta sürüme özel çalışır. Nokta
sürüm yükseltmek için tüm ağaç OpenWrt main ile yeniden senkronlanmalı (patches
sürümle birlikte gelir) — bu tek satırlık bir değişiklik DEĞİL, ayrı büyük iş.

**Çözüm:** Güncel nokta sürümü istiyorsan tüm ağacı OpenWrt main'den yeniden
çek + tüm target dosyaları (DTS, ipq53xx.mk, config-default, 02_network, patches)
yeniden uygula. Aksine bu oturumda (çalışan, test edilmiş) `.39` kalmak en güvenli.

**Doğrulama:** Dokümanlardaki kernel sürümü iddiasını YAZMADAN ÖNCE
`staging_dir/target-*/kernel.version` dosyasına bak (gerçek derlenen sürüm orada).
Doküman = gerçek build, tahmin değil.
