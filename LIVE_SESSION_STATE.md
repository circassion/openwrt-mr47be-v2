...
> ⚠️ Aşağıdaki imajlar şu an test4'ün kopyasıdır (SHA256 birebir aynı). Gerçek test5 build'i
> `bin/targets/qualcommbe/ipq53xx/` bittiğinde buraya kopyalanacak ve SHA256 DEĞİŞECEK.
...
- **Build başlatıldı:** 2026-08-29 (WSL, yeniden derleme — kernel + paketler)
- **Build yolu:** `/root/mr47be-clean`
- **Build komut:** `cd /root/mr47be-clean && setsid nohup make -j4 V=s > /root/mr47be-clean-build.log 2>&1 < /dev/null &`
- **Kernel:** 6.18.39 (patch'ler SORUNSUZ uygulandı, kernel derlendi → modül/paket aşaması)
- **Not:** .44/.48'e çıkma denemesi patch uyuşmazlığı ile BAŞARISIZ (bkz. Ders 5). 6.18.39 kaldı.
- **Çıktı:** bittiğinde `bin/targets/qualcommbe/ipq53xx/` → `imajlar/test5/`'e kopyala
